...
function calculateScore(board, cutCard) {
    ...
    for (const [i, row] of board.entries()) {
        ...
        score += rowScore;
        console.log(`Score for row: (Pairs: ${pairScore}) + (Runs: ${runScore}) + (Fifteens: ${fifteenScore}) + (Knobs: ${knobsScore}) + (Flushes: ${flushScore}) = ${rowScore}` // Updated console.log
        );
        rowScore = 0;
...
function calculateFlush(row) {
    ...
    for (const count of Object.values(suitCounts)) {
        if (count === 5)
            return 5; // full flush
        if (count === 4)
            flushScore = 4; // track 4-flush if no 5-flush
    }
    return flushScore;
}
