  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-800 p-4">
      ...
      <div className="bg-slate-700 p-8 rounded-lg shadow-xl w-[400px]">
        ...
        <p className="text-white text-lg mb-4">Game ID: <span className="font-bold text-cyan-300">{gameId}</span></p>
        <p className="text-white text-lg mb-4">Game Mode: <span className="font-bold text-cyan-300">{gameMode}</span></p>
        <p className="text-white text-lg mb-4">Players: {players.length} / {maxPlayers}</p>
        <ul className="list-disc list-inside text-white mb-6">
          {players.map((player) => (
            <li key={player.id}>{player.name} {player.id === hostId && "(Host)"}</li>
          ))}
        </ul>
